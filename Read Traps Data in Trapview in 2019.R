...
################################################################################
## NOTES for 2019
## 1- In 2019, the images were checked by Stephen Robinson and the results were 
##    saved in separate files for each location 
## 2- In NL, the pictures had funny times: 2:00-3:00, 6:30-7:30, 19:00-20:00 
##    and 22:30-23:30 so we have to make a separate set of day times
##    As a result DO NOT USE NL 2019 TRAP DATA FOR DIEL ACTIVITY !!!!
################################################################################
...
drive_download("Trap Monitoring 2019",path="./tmp/tmp2.xlsx",overwrite=TRUE)
...
D.2019 <- left_join(Traps,D$device,by=c("Code"="partNo")) %>%
...
D.2019 <- st_join(st_as_sf(D.2019,coords = c("Longitude",'Latitude'),crs=st_crs(state_prov),remove=FALSE)
                  ,st_as_sf(state_prov)) %>%
...
D.2019 <- D.2019 %>% mutate(Prov = replace(Prov,Code == "S04255" , "QC")) %>%
  mutate(Prov = replace(Prov,Code == "S04610" , "NL")) %>%
...
fromDate <- "2019-06-15T00:00:00"
toDate <- "2019-08-24T00:00:00"

# Download and read the data for each individual location
AllTraps <- do.call("rbind", lapply(D.2019$Location,function(x) {
  drive_download(paste0(x," 2019.xls"),path="./tmp/tmp.xls",overwrite=TRUE)
...
tmp <- AllTraps %>%
  dplyr::mutate(Hour=as.integer(format(dmy_hms(Date),"%H"))) %>%
...
Date1.noNL <- seq(from=ymd_hms("2019-06-15 01:30:00"), to=ymd_hms("2019-08-24 01:30:00"), by ="day")
Date2.noNL <- seq(from=ymd_hms("2019-06-15 09:30:00"), to=ymd_hms("2019-08-24 09:30:00"), by ="day")
Date3.noNL <- seq(from=ymd_hms("2019-06-15 13:30:00"), to=ymd_hms("2019-08-24 13:30:00"), by ="day")
Date4.noNL <- seq(from=ymd_hms("2019-06-15 21:30:00"), to=ymd_hms("2019-08-24 21:30:00"), by ="day")
tmp <- sort(c(Date1.noNL,Date2.noNL,Date3.noNL,Date4.noNL))
Dates.noNL <- tibble(StartDates=tmp[1:length(tmp)-1]) %>%
...
Date1.NL <- seq(from=ymd_hms("2019-06-15 04:00:00"), to=ymd_hms("2019-08-24 04:00:00"), by ="day")
Date2.NL <- seq(from=ymd_hms("2019-06-15 10:00:00"), to=ymd_hms("2019-08-24 10:00:00"), by ="day")
Date3.NL <- seq(from=ymd_hms("2019-06-15 20:00:00"), to=ymd_hms("2019-08-24 20:00:00"), by ="day")
Date4.NL <- seq(from=ymd_hms("2019-06-15 23:30:00"), to=ymd_hms("2019-08-24 23:30:00"), by ="day")
tmp <- sort(c(Date1.NL,Date2.NL,Date3.NL,Date4.NL))
Dates.NL <- tibble(StartDates=tmp[1:length(tmp)-1]) %>%
  mutate(EndDates=tmp[-1]) %>%
  mutate(TimeOfDay=rep_along(StartDates,c("Night","Morning","Afternoon","Evening"))) %>%
  mutate(FlightDay=if_else(TimeOfDay %in% c("Night","Morning"),lubridate::as_date(StartDates)-1,lubridate::as_date(StartDates)))
...
NewPests.f <- function(df,Dates) {
  ...
  tmp1 <- fuzzy_left_join(Dates,
                          df %>% mutate(Date=ymd_hms(dmy_hms(Date))),
                          by = c("StartDates"="Date", "EndDates"="Date"),
                          match_fun = list(`<=`, `>=`))
  tmp1 %>% mutate(Location=df$Location[1],NewPests=`Pests difference`) %>%
...
TrapCatches <- rbind(TrapCatches.NoNL,TrapCatches.NL)
...
saveWorkbook(wb, "Trap Monitoring 2019.xlsx", overwrite = TRUE)
