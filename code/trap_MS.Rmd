...
al <- all.locs[,c('Location', 'Longitude', 'Latitude', 'Year')]
...
provinces.sub <- subset(provinces, PT %in% provs)
valid.tf <- st_is_valid(provinces.sub)

## QC st geometry is invalid, so need to remove for intersection step
provs.valid <- provinces.sub$PT[valid.tf]
ps.new <- subset(provinces.sub, PT %in% provs.valid)

alu.df.st <- st_as_sf(alu.df, coords = c('Longitude', 'Latitude'), crs = 4326)
## Need to add QC back to alu.df at the end here
all.locs.rn <- st_intersection(alu.df.st, ps.new) %>%
  as.data.frame() %>%
  dplyr::select(Location, Location.orig, PT) %>%
  rename(Province = PT) %>%
  merge(alu.df, all.y = TRUE) %>%
  mutate(Province = ifelse(is.na(Province), 'QC', Province))
...
glm.df <- subset(glm.df.orig, Year != 2019 | Province != 'NF')
